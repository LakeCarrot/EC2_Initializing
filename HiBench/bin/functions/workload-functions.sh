...
function run-spark-job() {
	...
	SUBMIT_CMD="${SPARK_HOME}/bin/spark-submit ${LIB_JARS} --properties-file ${SPARK_PROP_CONF} --master ${SPARK_MASTER} ${YARN_OPTS} ${CLS} $@"
    	...
    	SUBMIT_CMD="${SPARK_HOME}/bin/spark-submit ${LIB_JARS} --executor-memory 6g --conf spark.eventLog.enabled=true --conf spark.eventLog.dir=/home/carrot/spark-1.4.1/conf/events --class ${CLS} --master spark://slave1:7077 ${YARN_OPTS} ${SPARKBENCH_JAR} $@"
    fi
