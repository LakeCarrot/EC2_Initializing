  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        // [Peizhen] define variable for task statistics
        var time: Long = 0
        ...
        for (m <- task.metrics) {
          ...
          // [Peizhen] set statistics for task 
          //m.updateShuffleReadMetrics();
          //m.updateInputMetrics();
          time = (taskFinish - taskStart) - task.executorDeserializeTime
          m.inputMetrics match {
            case Some(met) =>
              bytesRead = met.bytesRead
            case None => logError("[Peizhen] No bytesReadMetrics")
          }
          m.shuffleReadMetrics match {
            case Some(met) =>
              totalBytesRead = met.totalBytesRead
            case None => logError("[Peizhen] No totalBytesReadMetrics")
          }
          logError("[Peizhen] TaskStat: " + time + ", " + bytesRead + ", " + totalBytesRead)
        }
        ...
        // [Peizhen] replace with statusUpdateWithStat
        //execBackend.statusUpdate(taskId, TaskState.FINISHED, serializedResult)
        execBackend.statusUpdateWithStat(taskId, TaskState.FINISHED, serializedResult, time, bytesRead, totalBytesRead)
