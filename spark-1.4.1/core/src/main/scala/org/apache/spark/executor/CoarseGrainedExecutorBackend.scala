...
private[spark] class CoarseGrainedExecutorBackend(
  ...
  // [Peizhen] statusUpdateWithStat
  def statusUpdateWithStat(taskId: Long, state: TaskState, data: ByteBuffer, time: Long, bytesRead: Long, totalBytesRead: Long) {
    val msg = StatusUpdateWithStat(executorId, taskId, state, data, time, bytesRead, totalBytesRead)
    driver match {
      case Some(driverRef) => driverRef.send(msg)
      case None => logWarning(s"Drop $msg because has not yet connected to driver")
    }
